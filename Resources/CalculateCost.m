function [cost] = CalculateCost(calib, options, refCalibration, genPoints, detPoints, allcams_img_used)
...
for i = 1: numel(RTcnc1) % for each camera
    for n = 1: numel(RTpnc1{1}(1,1,:)) % for each image
        ...
        if (all(isnan(points(:))) == 0 && all(all(Rext))~=0)
            ...
            distortedPoints = allcams_img_used(n,i)*vision.internal.calibration.distortPoints(projectedPointsnorm, K, r_dist, t_dist);
